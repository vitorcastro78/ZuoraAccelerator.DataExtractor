...
namespace ZIP2GO.WebAPI.Controllers
{
    ...
    public class SubscriptionPlansController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("/subscription_plans/{subscriptionPlanId}")]
        ...
        public async Task<IActionResult> GetSubscriptionPlan([FromRoute][Required] string subscriptionPlanId, [FromQuery] string zuoraTrackId, bool async = true)
        {
            if (string.IsNullOrEmpty(subscriptionPlanId)) return BadRequest("subscriptionPlanId cannot be null");
            if (string.IsNullOrEmpty(zuoraTrackId)) return BadRequest("zuoraTrackId cannot be null");

            var subscriptionPlan = _subscriptionPlansService.GetSubscriptionPlanCached(subscriptionPlanId);
            
            if (subscriptionPlan is null)
            {
                subscriptionPlan = _subscriptionPlansService.GetSubscriptionPlan(subscriptionPlanId, zuoraTrackId, async); 
            }
        ...
        [SwaggerResponse(statusCode: 503, type: typeof(ErrorResponse), description: "Service Unavailable")]
        [SwaggerResponse(statusCode: 504, type: typeof(ErrorResponse), description: "Gateway Timeout")]
        public async Task<IActionResult> GetSubscriptionPlans([FromQuery] string cursor, string zuoraTrackId, bool async = true)
        {
            var subscriptionPlans = _subscriptionPlansService.GetSubscriptionPlansCached();

            if (subscriptionPlans.Data.Count() == 0) subscriptionPlans = _subscriptionPlansService.GetSubscriptionPlans( zuoraTrackId, async);
